...
//                  $sql="select * from place where sellersonid = {$userInfo}";
//                }else{
            $sql = "SELECT distinct p.id,p.* FROM `place` p , `media` m WHERE m.id=p.mediaid and p.sellerid = {$userInfo}";
//                  $sql="select * from place where sellerid = {$userInfo}";
//                    }
//            $sql="select * from place where sellerid = {$userInfo} ";
            $count = 0;
            //拼接搜索sql
...
//        }else{
        ...
        //2.查询所有媒体信息
        $medias = D("media")->where("status = 1 and   sellerid = {$_SESSION["userInfo"]['id']}")->select();
//        }
...
//        var_dump($palce['sellerid']);die;
    ...
    public function placeExport() {
        $userInfo = $_SESSION["userInfo"]['id']; //获取搜索条件
//        $name_seller = I("name_seller");                //媒体账户
        $name_media = I("name_media");                 //媒体名字
        $domain_media = I("domain_media");             //媒体域名
        $id_place = I("id_place");                     //广告位id
...
//    $sql="select * from place where sellerid = {$userInfo} ";
        $sql = "SELECT distinct p.id,p.* FROM `place` p , `media` m WHERE m.id=p.mediaid and p.sellerid = {$userInfo}";
//        $sql = "SELECT p.* FROM `place` p , `media` m, `seller` s WHERE m.id=p.mediaid AND s.id={$userInfo}";
        $count = 0;
        //拼接搜索sql
//        if($name_seller !=""){
//            $sql .= " and s.company like '%{$name_seller}%' ";
...
//                $v['nativeAssets'],
    ...
    public function placeImport() {
        ...
        if (!empty($_FILES)) {
            ...
            $userInfo = $_SESSION["userInfo"]['id'];
            $mes = "";
            if (($objPHPExcel->getActiveSheet()->getCell("A" . '1')->getValue()) != '媒体ID'
...
//                    $data['sellerSonId']            =    $media['sellersonid'];
                    ...
                    $data['deviceType'] = $objPHPExcel->getActiveSheet()->getCell("C" . $i)->getValue();
...
//                    $data['screenLocation'] = $objPHPExcel->getActiveSheet()->getCell("G" . $i)->getValue();
                    $data['placeType'] = $objPHPExcel->getActiveSheet()->getCell("D" . $i)->getValue();
                    $data['instl'] = $objPHPExcel->getActiveSheet()->getCell("E" . $i)->getValue();
                    $data['width'] = $objPHPExcel->getActiveSheet()->getCell("F" . $i)->getValue();
                    $data['height'] = $objPHPExcel->getActiveSheet()->getCell("G" . $i)->getValue();
                    $data['mimes'] = $objPHPExcel->getActiveSheet()->getCell("H" . $i)->getValue();
                    $data['fileExt'] = $objPHPExcel->getActiveSheet()->getCell("I" . $i)->getValue();
                    $data['maxduration'] = $objPHPExcel->getActiveSheet()->getCell("J" . $i)->getValue();
                    $data['minduration'] = $objPHPExcel->getActiveSheet()->getCell("K" . $i)->getValue();
                    $data['pos'] = $objPHPExcel->getActiveSheet()->getCell("L" . $i)->getValue();
                    $data['linearity'] = $objPHPExcel->getActiveSheet()->getCell("M" . $i)->getValue();
                    $data['code'] = $objPHPExcel->getActiveSheet()->getCell("N" . $i)->getValue();
                    $data['cuid'] = $userInfo;
                    $data['muid'] = $userInfo;
                    $data['ctime'] = $t;
                    $data['mtime'] = $t;
...
//                        }
//                    }
//                    if ($data['sellerId'] != $_SESSION["userInfo"]['id'] or $data['sellerSonId'] != $_SESSION["userInfo"]['id']) {
//                        $res = false;
//                    } else {

...
//            $idbuyeradvertiser = I('idbuyeradvertiser');
            $buyerAdvertiserId = I('buyerAdvertiserId');
            $id = I('id');
            $name = I('name');
            $status = I('status');
            ...
            $where = " 1";
            if($buyerName != ''){
                $dspID = D("Advertiser")->getDspId($buyerName);
                if(empty($dspID)){
                    $result['data'] = '';
                    $this->ajaxReturn($result);
                }else{
                    foreach($dspID as $k => $v){
                        $str[] = $v['id'];
                    }
                    $id_str = implode(",",$str);
                    $where.= ' and idBuyer in ('.$id_str.')';
                }
            }
            if($id != ''){
                $where.= ' and id ='.$id;
            }
            if($buyerAdvertiserId != ''){
                $where.= ' and idBuyerAdvertiser ='.$buyerAdvertiserId;
            }
            if($name != ''){
                $name = trim($name);

...
class IndexController extends Controller {
    public function index(){
        if (IS_POST) {//是否提交表单
            ...
            $password = I("password");
            $vcode=I('vcode');
            $userInfo =  D("user")->findByEmail($email);
            $verify = new \Think\Verify();
            $check_vcode = $verify->check($vcode);
           
            if(empty($userInfo)){
                $msg['status'] = 5;
                $msg['error_info'] = "用户不存在";
            } else {
                ...
                $this->ajaxReturn($msg);
                    ...
                    if($userInfo["status"] == 1){
                        ...
                        $msg['status'] = 3;
                        $msg['error_info'] = "用户审核不通过";
                    }else if($userInfo["status"] == 4){
                        $msg['status'] = 4;
                        $msg['error_info'] = "用户已经停用";
                    }
                }
            }
            $msg['vcode']=$vcode;
            $msg['check_vcode']=$check_vcode;
            $this->ajaxReturn($msg);
        }
        else{

...
class CreativeController extends BaseController {
    ...
    public function info(){
        ...
        foreach($audit as $k=>$v){
            switch($v['status']){
                case "1":
                    $audit[$k]['status'] = '<span class="label label-info arrowed arrowed-right">待上传</span>';
                    break;
                case "2":
                    $audit[$k]['status'] = '<span class="label label-warning arrowed arrowed-right">待审核</span>';
                    break;
                case "3":
                    $audit[$k]['status'] = '<span class="label label-success arrowed arrowed-right">审核通过</span>';
                    break;
                case "4":
//                    $audit[$k]['status'] = '<span data-rel="popover" title="'.$v['crid'].'" data-content="'.$v['errorid'].': '.$v['remark'].'">审核不通过</span>';
                    $audit[$k]['status'] = '<span class="label label-danger arrowed arrowed-right" data-rel="popover" title="'.$v['errorid'].': '.$v['remark'].'" data-content="'.$v['errorid'].': '.$v['remark'].'">审核不通过</span>';
//                    $audit[$k]['status'] = '<span class="label label-danger arrowed arrowed-right  title="'.$v['errorid'].': '.$v['remark'].'" data-content="'.$v['errorid'].': '.$v['remark'].'">审核不通过</span>';
                    break;
                case "5":
                    $audit[$k]['status'] = '<span class="label label-inverse arrowed arrowed-right">停用</span>';
                    break;
                case "6":
                    $audit[$k]['status'] = '<span class="label label-yellow arrowed arrowed-right">待重新上传</span>';
                    break;
            }
            switch($v['allow']){
                case "1":
                    $audit[$k]['allow'] = '<span class="label label-success arrowed-in arrowed-in-right">通过</span>';
                    break;
                case "2":
                    $audit[$k]['allow'] = '<span class="label label-danger arrowed-in arrowed-in-right">拒绝</span>';
                    break;
            }
        }
        if($list){
    ...
    public function mulitSetSellerAuditStatus() {
        $ret = ['status' => 1, 'msg' => '操作失败'];
        $creatives = trim(I('creatives'),',');
        $sellerId = I('sellerId');
        $allow = I('allow');
        ...
        $res = D('CreativeAudit')
            ->where("idSeller = $sellerId and crid in ($creatives)")
            ->save($data);

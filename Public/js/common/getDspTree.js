...
$.fn.appendcategoryHtml = function(node, boxid, ztreeObj, info) { //追趣
    ...
    hdiv.append('<input name="dsp[]" type="hidden" value="' + node.id +'"><span class="size-tag">' + node.name + '<span class="size-close" onclick="rmSize(this)">×</span></span>');
...
$.fn.checkallcategory = function(id, boxid, ztreeobj) {
    ...
    $("#" + id).bind("click", function() {
        $("#" + boxid).find("li[flag='dsp']").remove();
        $(znode).each(function(i, items) {
            $.fn.appendcategoryHtml(items, boxid, ztreeobj, "");
        });
    });
}
$.fn.uncheckalldsp = function(id, boxid, ztreeobj, flag) {
    var znode = ztreeobj.getNodes();
    $("#" + id).unbind("click");
    $("#" + id).bind("click", function() {
        $("#" + boxid + "").empty();
...
//    var myArray=[{"id":'51', "name":'121211','pId':'0'},{"id":'52', "name":'2322','pId':'0'}];  
    $.ajax({
        ...
        success: function(msg) {
            ...
            $.fn.uncheckalldsp("uncheckall_category", "categorytreebox", zTreeObjcategory, "category");
        }
